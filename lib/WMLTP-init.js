...
class modules_json {
    ...
    getTestCaseURL() {
        if (this.json.testCaseURL !== null) {
            return this.json.testCaseURL;
        } else {
            console.log("Incorrect network addr, will launch default addr, \n Please check config.json");
...
REMOTE_TEST_URL = MODULE_JSON.getTestCaseURL();
    ...
    if (WEBML_PREFER == "all") {
        if (TEST_PLATFORM == "linux") {
            if (WEBML_IEMYRIAD) {
                ...
                preferArray.push("Linux-WebNN-Fast-MKLDNN");
        ...
        if (TEST_PLATFORM == "linux") {
            if (WEBML_IEMYRIAD) {
                ...
                WEBML_PREFER == "Linux-WebNN-Fast-MKLDNN" ||
            ...
            WEBML_PREFER == "macOS-WebNN-Sustained-MPS") {
                preferArray.push(WEBML_PREFER);
            } else if (WEBML_PREFER == "macOS-WebNN-Fast-MKLDNN") {
                if (WEBML_SWITCH) {
                    preferArray.push(WEBML_PREFER);
                } else {
                    let str = WEBML_PREFER + " is incorrect because supportSwitch is false";
            ...
            WEBML_PREFER == "Win-WebNN-Fast-MKLDNN" ||
...
class modules_chrome {
    ...
    async create(prefer) {
        ...
        if (this.browserNewest) {
            ...
            if (prefer === "macOS-Polyfill-Fast-WASM") {
                this.remoteTestURL = REMOTE_TEST_URL + "?prefer=fast";
                ...
                this.remoteTestURL = REMOTE_TEST_URL + "?prefer=sustained";
                this.options = this.options
                    .setChromeBinaryPath(this.browserPath)
                    .addArguments("--disable-features=WebML");
            ...
            } else if (prefer === "macOS-WebNN-Fast-MKLDNN") {
                // Add process ENV
                process.env.LD_LIBRARY_PATH = this.getLDLibraryPath();
                this.remoteTestURL = REMOTE_TEST_URL + "?prefer=fast";
                this.options = this.options
                    .setChromeBinaryPath(this.browserPath)
                ...
                this.options = this.options
                    .setChromeBinaryPath(this.browserPath)
                    .addArguments("--disable-features=WebML")
            } else if (prefer === "Win-WebNN-Fast-MKLDNN") {
                this.remoteTestURL = REMOTE_TEST_URL + "?prefer=fast";
                this.options = this.options
                    .setChromeBinaryPath(this.browserPath)
                    .addArguments("--enable-features=WebML");
                ...
                this.remoteTestURL = REMOTE_TEST_URL + "?prefer=low";
                this.options = this.options
                    .setChromeBinaryPath(this.browserPath)
                    .addArguments("--use-dml")
                    .addArguments("--enable-features=WebML");
                ...
                this.options = this.options
                    .setChromeBinaryPath(this.browserPath)
                    .addArguments("--disable-features=WebML");
            } else if (prefer === "Linux-WebNN-Fast-MKLDNN") {
                this.remoteTestURL = REMOTE_TEST_URL + "?prefer=fast";
                this.options = this.options
                    .setChromeBinaryPath(this.browserPath)
                    .addArguments("--enable-features=WebML")
    ...
    getLDLibraryPath() {
        let basePath = this.browserPath.slice(0, -14) + "Versions/";
...
class modules_csv {
    ...
    async open(prefer) {
        ...
        if (TEST_PLATFORM == "android") {
            ...
            name = GET_REPORT_PATH() + path.sep + "report-" + prefer + "-" + CPUType + "-" + TOOLS_DATE() + ".csv";
        } else {
            name = GET_REPORT_PATH() + path.sep + "report-" + prefer + "-" + TOOLS_DATE() + ".csv";
        }
