...
//    "Android-WebNN-Low-NNAPI",
    ...
    "Win-WebNN-Fast-DNNL",
    "Win-WebNN-Fast-IE-MKLDNN",
    "Win-WebNN-Sustained-DML",
    "Win-WebNN-Sustained-IE-clDNN",
    "Win-WebNN-Sustained-clDNN",
    "Win-WebNN-Low-DML",
    "Win-WebNN-Low-IE-MYRIAD",
    "Win-WebNN-Ultra-Low-IE-GNA",
...
class modules_json {
    ...
    getWebmlSwitch() {
        if (typeof(this.json.webml.switch.linux.INFERENCE_ENGINE) == "boolean" &&
            typeof(this.json.webml.switch.linux.BACKEND_LIST) == "object" &&
            typeof(this.json.webml.switch.mac.DNNL) == "boolean" &&
            typeof(this.json.webml.switch.windows.DML) == "boolean" &&
            typeof(this.json.webml.switch.windows.INFERENCE_ENGINE) == "boolean" &&
            typeof(this.json.webml.switch.windows.BACKEND_LIST) == "object") {
            return this.json.webml.switch;
        } else {
            let string = "Can not support this WebML switch: " + this.json.webml.switch;
            throw new Error(string);
        }
    }
...
GET_PREFER_MODELS = function() {
    ...
    if (WEBML_PREFER == "all") {
        if (TEST_PLATFORM == "linux") {
            ...
            if (WEBML_SWITCH.linux.INFERENCE_ENGINE) {
                if (WEBML_SWITCH.linux.BACKEND_LIST.length !== 0) {
                    for (let backend of WEBML_SWITCH.linux.BACKEND_LIST) {
                        if (backend === "IE-MKLDNN") preferArray.push("Linux-WebNN-Fast-IE-MKLDNN");
                        else if (backend === "IE-clDNN") preferArray.push("Linux-WebNN-Sustained-IE-clDNN");
                        else if (backend === "IE-MYRIAD") preferArray.push("Linux-WebNN-Low-IE-MYRIAD");
                        else if (backend === "IE-GNA") preferArray.push("Linux-WebNN-Ultra-Low-IE-GNA");
            ...
            if (WEBML_SWITCH.windows.DML) {
                preferArray.push("Win-WebNN-Sustained-DML");
                preferArray.push("Win-WebNN-Low-DML");
            }

            if (WEBML_SWITCH.windows.INFERENCE_ENGINE) {
                if (WEBML_SWITCH.windows.BACKEND_LIST.length !== 0) {
                    for (let backend of WEBML_SWITCH.windows.BACKEND_LIST) {
                        if (backend === "IE-MKLDNN") preferArray.push("Win-WebNN-Fast-IE-MKLDNN");
                        else if (backend === "IE-clDNN") preferArray.push("Win-WebNN-Sustained-IE-clDNN");
                        else if (backend === "IE-MYRIAD") preferArray.push("Win-WebNN-Low-IE-MYRIAD");
                        else if (backend === "IE-GNA") preferArray.push("Win-WebNN-Ultra-Low-IE-GNA");
        ...
        if (TEST_PLATFORM == "linux") {
            if (WEBML_PREFER == "Linux-Polyfill-Fast-WASM" ||
                ...
                preferArray.push(WEBML_PREFER);
                       ...
                       WEBML_PREFER == "Linux-WebNN-Ultra-Low-IE-GNA" ||
                       WEBML_PREFER == "Linux-WebNN-Low-IE-MYRIAD") {
                if (WEBML_SWITCH.linux.INFERENCE_ENGINE) {
                    preferArray.push(WEBML_PREFER);
                } else {
                    let str = WEBML_PREFER + " is incorrect because 'INFERENCE_ENGINE' switch is false";
                    throw new Error(str);
                }
            } else {
            ...
            if (WEBML_PREFER == "macOS-Polyfill-Fast-WASM" ||
                ...
                if (WEBML_SWITCH.mac.DNNL) {
                    ...
                    let str = WEBML_PREFER + " is incorrect because 'DNNL' switch is false";
                    throw new Error(str);
                }
            } else {
                let str = WEBML_PREFER + " is incorrect because test platform is " + TEST_PLATFORM;
            ...
            if (WEBML_PREFER == "Win-Polyfill-Fast-WASM" ||
                ...
                if (WEBML_SWITCH.windows.DML) {
                    ...
                    let str = WEBML_PREFER + " is incorrect because 'DML' switch is false";
                    throw new Error(str);
                }
            } else if (WEBML_PREFER == "Win-WebNN-Fast-IE-MKLDNN" ||
                       WEBML_PREFER == "Win-WebNN-Sustained-IE-clDNN" ||
                       WEBML_PREFER == "Win-WebNN-Low-IE-MYRIAD" ||
                       WEBML_PREFER == "Win-WebNN-Ultra-Low-IE-GNA") {
                if (WEBML_SWITCH.windows.INFERENCE_ENGINE) {
                   preferArray.push(WEBML_PREFER);
                } else {
                   let str = WEBML_PREFER + " is incorrect because 'INFERENCE_ENGINE' switch is false";
        ...
        if (this.browserNewest) {
            ...
            if (prefer === "macOS-Polyfill-Fast-WASM") {
                ...
                this.options = this.options
                    ...
                    .addArguments("--enable-features=WebML");
            } else if (prefer === "Win-WebNN-Sustained-IE-clDNN") {
                this.remoteTestURL = CURRENT_URL + "?prefer=sustained";
                this.options = this.options
                    .setChromeBinaryPath(this.browserPath)
                    .addArguments("--no-sandbox")
                ...
                this.options = this.options
                    ...
                    .addArguments("--use-dml")
                    .addArguments("--enable-features=WebML");
            } else if (prefer === "Win-WebNN-Low-IE-MYRIAD") {
                this.remoteTestURL = CURRENT_URL + "?prefer=low";
                this.options = this.options
                    .setChromeBinaryPath(this.browserPath)
                    .addArguments("--no-sandbox")
                ...
                this.remoteTestURL = CURRENT_URL + "?prefer=Ultra-Low";
                this.options = this.options
                    .setChromeBinaryPath(this.browserPath)
                    .addArguments("--no-sandbox")
                    .addArguments("--use-inference-engine")
                ...
                this.options = this.options
                    ...
                    .addArguments("--use-inference-engine")
                    .addArguments("--no-sandbox");
            } else if (prefer === "Linux-WebNN-Ultra-Low-IE-GNA") {
                this.remoteTestURL = CURRENT_URL + "?prefer=Ultra-Low";
                this.options = this.options
                    .setChromeBinaryPath(this.browserPath)
                    .addArguments("--enable-features=WebML")
