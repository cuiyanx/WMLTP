...
class modules_json {
    ...
    getWebmlSwitch() {
        if (typeof(this.json.webml.flag) == "boolean") {
            return this.json.webml.flag;
        } else {
            let string = "Can not support this WebML switch: " + this.json.webml.flag;
            throw new Error(string);
        }
    }

    getWebmlBackend() {
        if (this.json.webml.flag === false) {
            if ((this.json.webml.backend == "webgl2") || (this.json.webml.backend == "wasm")) {
                return this.json.webml.backend;
            } else {
                let string = "Can not support this backend: " + this.json.webml.backend + ", webml polyfill only support webgl2/wasm.";
                throw new Error(string);
            }
        }
}
...
WEBML_BACKEND = MODULE_JSON.getWebmlBackend();
    ...
    async open() {
        ...
        if (TEST_PLATFORM == "android") {
            ...
            if (WEBML_SWITCH === false) {
                name = GET_REPORT_PATH() + "/report-" + TEST_PLATFORM + "-" + cpu + "-" + "webml-" + WEBML_SWITCH + "-backend-" + WEBML_BACKEND + "-" + TOOLS_DATE() + ".csv";
            } else {
                name = GET_REPORT_PATH() + "/report-" + TEST_PLATFORM + "-" + cpu + "-" + "webml-" + WEBML_SWITCH + "-backend-" + "default" + "-" + TOOLS_DATE() + ".csv";
            }
        } else {
            if (WEBML_SWITCH === false) {
                name = GET_REPORT_PATH() + "/report-" + TEST_PLATFORM + "-" + "webml-" + WEBML_SWITCH + "-backend-" + WEBML_BACKEND + "-" + TOOLS_DATE() + ".csv";
            } else {
                name = GET_REPORT_PATH() + "/report-" + TEST_PLATFORM + "-" + "webml-" + WEBML_SWITCH + "-backend-" + "default" + "-" + TOOLS_DATE() + ".csv";
            }
        }
