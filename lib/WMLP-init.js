...
class modules_json {
    ...
    open() {
        this.json = JSON.parse(fs.readFileSync("./WMLP.config.json"));
    }
    ...
    write() {
        fs.writeFileSync("./WMLP.config.json", JSON.stringify(this.json, null, 4));
    }
    ...
    async create() {
        if (this.browserNewest) {
            if (TEST_PLATFORM === "linux") {
                ...
                this.options = new this.chrome.Options().androidPackage("org.chromium.chrome");
            }
        } else {
...
class modules_csv {
    ...
    async open() {
        ...
        let name = GET_REPORT_PATH() + "/report-" + TEST_PLATFORM + "-" + TOOLS_DATE() + ".csv";
...
class modules_tools {
    ...
    install(path) {
        ...
        if (RUN_PLATFORM === "linux") {
            if (TEST_PLATFORM === "linux") {
                ...
                let command = "adb install -r -d " + path;
                let subprocess = this.childProcess.execSync(command, {encoding: "UTF-8"});
    ...
    checkCommit(commit) {
        let str = "./output/package/" + commit + "/";
        return fs.existsSync(str);
    }

    checkPackage(commit) {
        let str = "./output/package/" + commit + MODULE_JSON.getPath() + MODULE_JSON.getPackage();
        return fs.existsSync(str);
    }
