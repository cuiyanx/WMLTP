...
if (sys == "Linux") {
    ...
    throw new Error("We do not support " + sys + " as run platform");
}
...
if (WMLPjson.platform == "linux" ||
    WMLPjson.platform == "android" ||
    WMLPjson.platform == "mac" ||
    WMLPjson.platform == "ios" ||
    WMLPjson.platform == "windows") {
    TEST_PLATFORM = WMLPjson.platform;
} else {
    throw new Error("Can not support this test platform: " + WMLPjson.platform);
}
...
TOOLS_TIME = function () {
    ...
    if (RUN_PLATFORM == "mac") {
        TimeString = arrayDate[0] + arrayDate[1] + arrayDate[2];
    } else {
        TimeString = arrayDate[2] + arrayDate[0] + arrayDate[1];
    }
...
class modules_json {
    ...
    writePackage(name) {
        ...
        if (TEST_PLATFORM == "linux") {
            this.json.package.linux = name;
        } else if (TEST_PLATFORM == "mac") {
            this.json.package.mac = name;
        } else if (TEST_PLATFORM == "windows") {
            this.json.package.windows = name;
        } else if (TEST_PLATFORM == "android") {
    ...
    async create() {
        if (this.browserNewest) {
            if (TEST_PLATFORM === "linux") {
                ...
                this.browserPath = "./output/package/chromium-mac/Chromium.app/Contents/MacOS/Chromium";
                this.options = new this.chrome.Options().setChromeBinaryPath(this.browserPath);
            } else if (TEST_PLATFORM === "android") {
...
class modules_tools {
    ...
    async download(URL) {
        ...
        if (RUN_PLATFORM === "linux") {
            ...
            let command = "curl -o " + PACKAGE_PATH + MODULE_JSON.package + " " + URL;
            await this.childProcess.execSync(command, {stdio: "inherit"});
        }
    }
    ...
    install(path) {
        ...
        if (RUN_PLATFORM === "linux") {
            ...
            if (TEST_PLATFORM === "mac") {
                let unzipPath = "./output/package/chromium-mac/";
